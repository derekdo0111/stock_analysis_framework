...
- 5规则YAML/文档驱动（hard_gate / l2_screener / turtle_constants / agent_constraints / cv_methodology_brief）
- Agent约束：分析Agent(CFA)三段式证据链 + 验证Agent(CPA+CFE)10项审计程序 + CV Agent(事实核查员，按声明类型分策略核查)

## 方法论 v0.33 DC 纯流量修正（2026-06-04）

### 问题
v0.22 DC 公式混合了流量项和存量项：
```
DC = op_cf - maintenance_capex - acq_subsidiary - equity_invest - fin_expense
     + money_cap - restricted_cash - st_borr + trad_assets
```
`money_cap`(年末货币资金) 是存量，已包含当年经营产生的现金流。`+op_cf` + `+money_cap` 形成双重计数。格力电器 DC 1131亿→实际约450亿，PR 25.18%→约10%。

### 修正
纯流量公式：
```
DC = op_cf - maintenance_capex - acq_subsidiary - equity_invest_increase - fin_expense
```
删除 `+ money_cap - restricted_cash - st_borr + trad_assets` 四个存量项。
L5 内部也调用 DisposableCashCalculator，合理市值和安全边际率同步修正。
