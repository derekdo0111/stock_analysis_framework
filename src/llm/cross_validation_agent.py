...
from src.llm.claim_types import AnalysisClaim, VerifiedClaim
...
class CrossValidationAgent:
    ...
    def _load_methodology() -> str:
        ...
        # 尝试多个可能的路径
        candidates = [
            Path("rules/cv_methodology_brief.md"),
            Path(__file__).parent.parent.parent / "rules" / "cv_methodology_brief.md",
        ]
        for p in candidates:
            if p.exists():
