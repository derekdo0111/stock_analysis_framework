...
from src.turtle.rules.schemas import BusinessModelConfig
...
class L3Calculator:
    ...
    def __init__(self, bundle: StockDataBundle, management_signal: str = "data_unavailable"):
        self._bundle = bundle
        ...
        self._management_signal = management_signal  # Phase 3.5 LLM 后修正信号
    ...
    def _eval_management_stability(self, dim) -> tuple[float, str]:
        """管理层稳定性: 优先使用 LLM 商业检索信号，降级默认 1 分。"""
        signal = self._management_signal
        if signal == "stable":
            return 2, "管理层稳定（LLM 确认）"
        elif signal == "frequent_change":
            return 0, "管理层频繁变更（LLM 检出）"
        elif signal in ("unclear", "data_unavailable"):
            return 1, "数据不足（默认中性）"
        # 未知信号值：保守给 1 分
        return 1, f"数据不足（默认中性）"
