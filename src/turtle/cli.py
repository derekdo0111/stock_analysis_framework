...
def main():
    parser = argparse.ArgumentParser(
        description="Augur v0.36 — 多策略A股分析框架 · 古罗马占卜官不靠水晶靠数据",
    )
    ...
    print(f"[Augur] v0.36 -- Analyzing {args.ts_code}")
    print("=" * 60)
    ...
    print("\n[Phase 3] 财报深度分析 (7模块)...")
    from src.turtle.calculator.financial_deep_analysis import FinancialDeepAnalyzer
    try:
    ...
    if args.no_llm:
        ...
        if not LLMConfig.is_configured():
            ...
            from src.turtle.llm.business_retrieval_agent import BusinessRetrievalAgent
            ...
            try:
                ...
                if business_knowledge.success:
                    ...
                    # v0.34: Phase 3.5 后修正 L3 管理层稳定性维度
                    signal = business_knowledge.management_stability_signal
                    _SIGNAL_SCORE_MAP = {"stable": 2, "frequent_change": 0}
                    mgmt_score = _SIGNAL_SCORE_MAP.get(signal, 1)  # unclear/data_unavailable → 1
                    mgmt_labels = {
                        2: "管理层稳定（LLM 确认）",
                        0: "管理层频繁变更（LLM 检出）",
                        1: "数据不足（默认中性）",
                    }
                    old_mgmt = result.l3_dim_scores.get("management_stability", {}).get("score", 1)
                    if mgmt_score != old_mgmt:
                        result.adjust_management_stability(mgmt_score, mgmt_labels[mgmt_score])
                        print(f"  [L3 修正] management_stability: {old_mgmt}→{mgmt_score} "
                              f"({signal}), L3={result.l3_score}/30, Final={result.final_score}/100")
                    else:
                        print(f"  [L3] management_stability 无需修正 (signal={signal})")
                else:
            ...
            from src.turtle.llm.analysis_agent import AnalysisAgent
            analysis_text = ""
            try:
                aa = AnalysisAgent()
            ...
            from src.turtle.llm.claim_types import ClaimVerificationResult
            ...
            if analysis_text:
                ...
                if claims:
                    ...
                    from src.turtle.llm.cross_validation_agent import CrossValidationResult, Discrepancy
    ...
    if not args.no_llm and analysis_result and analysis_result.success:
        ...
        if cv_issues_list:
            ...
            from src.turtle.llm.analysis_agent import AnalysisAgent as RCAgent
            try:
                rc_agent = RCAgent()
