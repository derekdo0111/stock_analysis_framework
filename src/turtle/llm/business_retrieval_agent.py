...
class BusinessKnowledgeResult:
    ...
    # 管理层稳定性信号（用于 L3 后修正）
    management_stability_signal: str = "data_unavailable"  # stable | unclear | frequent_change | data_unavailable
...
## 输出格式（严格 JSON）
{
  ...
  "management_stability_signal": "stable|unclear|frequent_change",
  "source_urls": ["https://...", "https://..."]
}

management_stability_signal 说明:
- "stable": 近2-3年管理层(董事长/总经理/CFO/核心高管)未发生重大变更，治理结构稳固
- "frequent_change": 近2-3年有多次核心高管离职/更换/被调查，治理不稳定
- "unclear": 无法确定或仅有模糊信息，默认返回此值"""
...
management_stability_signal 说明:
- "stable": 管理层(董事长/总经理/CFO)近2-3年未发生重大变更
- "frequent_change": 有多次核心高管离职/更换
- "unclear": 无法确定，默认返回此值"""
...
class BusinessRetrievalAgent:
    ...
    def retrieve(
        ...
        # 检查搜索可用性
        from src.core.llm.tools import _search_provider
    ...
    def _retrieve_with_search(
        ...
        from src.core.llm.tools import BUSINESS_RETRIEVAL_TOOLS, execute_tool
    ...
    def _populate_result(result: BusinessKnowledgeResult, data: dict) -> None:
        ...
        result.management_stability_signal = data.get("management_stability_signal", "data_unavailable")
