...
| 阶段二 (确定性管线) | 🟢 完成 | HardGate(6项否决) + L2初筛(20分) + 公司5分类 + OE路径B + 穿透回报率(v0.21修正分配比率外推) + L5安全边际(6维外推+5陷阱+3×3仓位矩阵) + 乘法打分 |
...
**当前版本**: v0.21 — PR = (可支配现金 × 分配比率 × 0.9 + 回购注销) / 当前市值
