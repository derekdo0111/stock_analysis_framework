...
在 `D:\project\stock-analysis-framework\` 创建 Python 股票分析框架，融合 **龟龟投资策略框架（Turtle Strategy v0.19）** 完整方法论——从 HardGate 一票否决、L2 分层初筛、公司分类、穿透回报率计算（分红+回购）到乘法打分模型，实现从选股到报告生成的完整分析管线。系统采用确定性计算与 LLM 智能分析分离的架构，以 **Tushare（2000积分会员）** 为主要数据源，覆盖 **22 个接口**（8 财报 + 6 行情 + 8 股东治理）+ 行业数据，辅以 Web 搜索，通过 2 个 LLM Agent（分析+验证）配合 Jinja2 模板渲染输出 HTML+MD 格式的股票分析报告。
...
Final Score = (L2初筛 20pt + L4穿透回报率 40pt + L5安全边际 25pt) × L3商业模式乘数（优×1.2/良×1.0/中×0.8/差截断）。L4 v0.19前瞻公式：PR = (可支配现金 × 分配比率 × 0.90 + 回购注销金额) / 当前市值，分配比率两级降级（公告承诺×0.8 / 历史外推×0.7）。OE=经营CF净额−总CAPEX×维持系数(5年中位数)，OE质量标签前置(可信/存疑×0.7/不可靠→0)，质量四级验证扣分后≤40分上限。≥75核心池，55~74观察池，<55备选池

### 龟龟策略核心指标

**Owners' Earnings（路径B，v0.19单路径）**：OE_cf = 经营性现金流净额 − 总CAPEX × 维持系数(5年中位数，用于OE质量验证)。维持性CAPEX系数采用三因子评估法：行业先验(权重40%) × 资产轻重评分(权重60%)。v0.19 删除路径A(OE_income)，因其折旧摊销用CAPEX×0.55估计，数据不可靠。

**穿透回报率（v0.19前瞻公式）**：PR = (当前可支配现金 × 分配比率 × (1-10%红利税) + 回购注销金额) / 最新收盘总市值。可支配现金 = 经营CF净额 − 投资活动现金流出小计 − 财务费用 + 货币资金 − 限制性货币 − 短期借款 + 交易性金融资产。分配比率：一档(公告分红承诺×0.8) / 二档(历史可支配现金分红率中位数×0.7)。回购注销：Web+LLM提取，仅确认用于注销的回购。三级阈值起点分(12%/8%/5%)，OE质量三级标签前置(🟢可信/🟡存疑×0.7/🔴不可靠→L4=0)，OE质量四级验证扣分(含金量+稳定性+趋势+BS一致性)，上限40分。
...
Tushare 核心接口 + akshare 备用 + Web+LLM 提取（Layer3）→ 多源编排器 + tenacity重试；19 Pydantic v2模型双轨Schema（核心字段+RawTushareData全量）；JSON+Parquet双格式存储；数据通过 StockDataBundle 统一注入，所有计算模块只读 bundle
...
五阶段递进，先确定性层（Python），后智能层（LLM），每阶段产出可测试最小单元。

- **阶段一（地基）✅**：项目骨架+基础设施层+19 Pydantic Schema+数据校验存储+3规则YAML
- **阶段二（确定性管线）✅**：HardGate+L2初筛分类+Tushare接口+数据池转换+OE路径B+穿透回报率(v0.19)+L5安全边际+乘法打分+报告渲染+集成测试
...
│   ├── data_fetcher/ (base+web+web_extractor+orchestrator)
│   ├── screener/
...
│   │   ├── registry.py               # 策略注册表
│   │   ├── turtle_strategy/
│   │   │   ├── oe_calculator.py      # OE路径B计算+CAPEX资产轻重评估(v0.19单路径)
│   │   │   ├── pr_calculator.py      # 穿透回报率(v0.19前瞻公式)+OE质量扣分
│   │   │   ├── l5_calculator.py      # 安全边际+3×3仓位矩阵+价值陷阱5项排查
